...
2. Si existe `./init.sh`, ejecutalo antes de tocar codigo. Si falla, para y
   resuelve el entorno antes de modificar archivos.
...
6. Si el repo activo es de codigo y faltan `tests/`, `docs/` o `progress/`,
   inicializa antes de implementar la estructura minima, salvo contradiccion
   local: `tests/.gitkeep`; `docs/{architecture,conventions,verification}.md`;
   `progress/{Current,History,Decisions,Blockers}.md`.
